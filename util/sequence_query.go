...
/*
Just for query sequence as JSON.
This will be depreciated.
*/

func NewSequenceQueryHandler(keyStore sdk.KVStoreKey, decoder sdk.AccountDecoder) types.QueryHandler {
	return func(baseapp *bam.BaseApp, req abci.RequestQuery) (res abci.ResponseQuery) {
		...
		m["sequence"] = account.GetSequence()
		res.Value, err = json.Marshal(m)
		if err != nil {
			return sdk.ErrUnknownRequest(err.Error()).Result().ToQuery()
		}
		return
