...
func defaultParams() powerTickParam {
	return powerTickParam{
		TotalPowerSupply: sdk.NewRat(10000000000000000, 1),
		UnusedSupply:     sdk.NewRat(0),
		InflationRate:    sdk.NewRat(1, 10),
	}
}

func createTestInput(t *testing.T, isCheckTx bool) (sdk.Context, PowerKeeper, sdk.AccountMapper, bank.CoinKeeper) {
	db := dbm.NewMemDB()
	...
	powerKeeper := NewPowerKeeper(keyMain, cdc, accountMapper, coinKeeper)
	powerKeeper.SetPowerTickParam(ctx, defaultParams())

	for _, addr := range addrs {
		coinKeeper.AddCoins(ctx, addr, sdk.Coins{
			sdk.Coin{Denom: "quokki", Amount: defaultParams().TotalPowerSupply.Evaluate() / int64(len(addrs))},
		})
		powerKeeper.PowerUp(ctx, addr, (defaultParams().TotalPowerSupply.Evaluate()/int64(len(addrs)))/2)
	}

	return ctx, powerKeeper, accountMapper, coinKeeper
}
